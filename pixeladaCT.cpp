  ...
  for(i=0;i<nSlaves;i++)
    {
      ...
      std::cout << "INFO: (inside pixeladaCT::pixeladaCT) new commDaemon instance" << std::endl;
...
int pixeladaCT::dataToComm()
{
  //std::cout << "DEBUG: (inside pixeladaCT::dataToComm)" << std::endl;
  int failed = -1;
  int tableFail = 0;
  ...
  for(int i=0; i< nDBs ; i++)
      ...
      for(int j=0; j < tablesList.size() ; j++)
	{
	  ...
	  std::cout << "DEBUG: (inside pixeladaCT::dataToComm) tableFail:"<< tableFail << std::endl;
	  //resetting trigger
	  if(!tableFail)
	    {
...
int pixeladaCT::dataToDB()
{
  //std::cout << "DEBUG: (inside pixeladaCT::dataToDB)" << std::endl;
  int failed = -1;
  std::vector<int> tablesList;
  field currentTag;
  int indexTag;
...
int pixeladaCT::lockTables()
{
  //std::cout << "DEBUG:(inside pixeladaCT::lockTables)" << std::endl; 
  int failed = -1;
...
int pixeladaCT::getTriggers()
{
  //std::cout << "DEBUG:(inside pixeladaCT::getTriggers)" << std::endl; 
  int failed = -1;
...
int pixeladaCT::delTriggers()
{
  //std::cout << "DEBUG:(inside pixeladaCT::delTriggers)" << std::endl; 
  int failed = -1;
  ...
  //std::cout << "DEBUG:(inside pixeladaCT::delTriggers) returning" << std::endl; 
  return failed;
}

//DEBUG FUNCTIONS
/*show data in memory structures, data to be written in databases*/
int pixeladaCT::showDBData()
{
  int slave = 0;
  int field = 0;
  //databases
...
int pixeladaCT::showDBDataLinkage()
{
  std::cout <<"DEBUG:(inside pixeladaCT::showDBDataLinkage)" << std::endl;
...
/*show triggers ON*/
int pixeladaCT::showTriggers()
{
  std::cout <<"DEBUG:(inside pixeladaCT::showTriggers)" << std::endl;
   //databases
  for(int i=0; i< nDBs ; i++)
    {
      std::cout << "**************************************************" << std::endl;
...
void pixeladaCT::sigsevHandler(int s)
{
  void *array[10];
  size_t size;
